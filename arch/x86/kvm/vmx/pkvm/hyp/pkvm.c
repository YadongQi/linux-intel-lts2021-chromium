			    ...
			    size_t shadow_size)
{
	...
	ret = read_gva(hvcpu, vcpu_va, &shadow_vcpu->vmx, sizeof(struct vcpu_vmx), NULL);
	if (ret != sizeof(struct vcpu_vmx))
		return -EINVAL;

	vmcs12_va = (unsigned long)shadow_vcpu->vmx.vmcs01.vmcs;
	if (gva2gpa(hvcpu, vmcs12_va, (gpa_t *)&shadow_vcpu->vmcs12_pa, 0, NULL))
		return -EINVAL;
