				...
				struct pkvm_pgtable_map_data *data)
{
	...
	if (data->map_leaf_override)
		return data->map_leaf_override(pgt, vaddr, level, ptep, data);
	else
		return pgtable_map_leaf(pgt, vaddr, level, ptep, data);
}
				 ...
				 struct pkvm_pgtable_map_data *data)
{
	...
	ret = pgtable_map_try_leaf(pgt, vaddr, vaddr_end, level, ptep, data);
	if (ret != -E2BIG)
		return ret;
...
static int pgtable_free_leaf(struct pkvm_pgtable *pgt, void *ptep)
{
			    ...
			    void * const arg)
{
	...
	if (pgt_ops->pgt_entry_is_leaf(ptep, level)) {
		if (data->free_leaf_override)
			return data->free_leaf_override(pgt, vaddr, level, ptep, data);
		else
			return pgtable_free_leaf(pgt, ptep);
	}
...
static int pgtable_visit(struct pgt_walk_data *data, void *ptep, int level)
{
	...
	if (!leaf && (flags & PKVM_PGTABLE_WALK_TABLE_PRE))
		ret = walker->cb(data->pgt, data->vaddr, data->vaddr_end, level,
				 ptep, PKVM_PGTABLE_WALK_TABLE_PRE, walker->arg);

	if (leaf && (flags & PKVM_PGTABLE_WALK_LEAF)) {
		ret = walker->cb(data->pgt, data->vaddr, data->vaddr_end, level,
				 ptep, PKVM_PGTABLE_WALK_LEAF, walker->arg);
	...
	if (flags & PKVM_PGTABLE_WALK_TABLE_POST)
		ret = walker->cb(data->pgt, data->vaddr, data->vaddr_end, level,
				 ptep, PKVM_PGTABLE_WALK_TABLE_POST, walker->arg);
			...
			struct pkvm_pgtable_walker *walker)
{
	...
	return _pgtable_walk(&data, mm_ops->phys_to_virt(pgt->root_pa), pgt->level);
}
