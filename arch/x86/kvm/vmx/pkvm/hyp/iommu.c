...
{
	struct viommu_reg *vreg = &iommu->viommu.vreg;
	u32 changed = ((vreg->gsts ^ val) & DMAR_GCMD_DIRECT) &
		      DMAR_GSTS_EN_BITS;
	u32 set = (val & DMAR_GCMD_DIRECT) & ~DMAR_GSTS_EN_BITS;
	u32 cmd, gcmd, sts;
	int bit;

	if ((changed | set) & DMAR_GCMD_PROTECTED) {
		pkvm_dbg("pkvm:%s touching protected bits changed 0x%x set 0x%x\n",
			 __func__, changed, set);
		return;
	}

	if (changed) {
		pkvm_dbg("pkvm: %s: changed 0x%x\n", __func__, changed);
		gcmd = READ_ONCE(iommu->iommu.gcmd);
		for_each_set_bit(bit, (unsigned long *)&changed, 32) {
			cmd = 1 << bit;
			if (val & cmd) {
				/* enable */
				gcmd |= cmd;
	...
	if (set) {
		pkvm_dbg("pkvm: %s: set 0x%x\n", __func__, set);
		gcmd = READ_ONCE(iommu->iommu.gcmd);
		for_each_set_bit(bit, (unsigned long *)&set, 32) {
			cmd = 1 << bit;
			vreg->gsts &= ~cmd;
